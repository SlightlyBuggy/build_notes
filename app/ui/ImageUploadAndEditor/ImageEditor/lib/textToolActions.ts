...
export const mouesDownTextTool = (currentX: number, currentY: number, 
    ...
    if(textInputState.active)
    {
        if(textInputState.value)
        {
            ...
            // TODO: need to 
            const objectBoundaries = getObjectBoundaries({toolType: command.drawingTool, startX: command.startX, startY: textInputState.posY, endX: endX, endY: endY})
        ...
        textInputStateSetter({active: false, posX: textInputState.posX, posY: currentY, value: '', width: 0, height: 0})
        return
    }
    // TODO: we shouldn't care about setting width, height, and value here.  may need a helper function
    textInputStateSetter({active: true, posX: currentX, posY: currentY, value: '', width: 0, height: 0})
}
