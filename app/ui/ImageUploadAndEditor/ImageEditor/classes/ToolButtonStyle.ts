...
export const toolButtonStyleFactory = (
  thisToolType: DrawingTool,
  activeToolType: DrawingTool | null
): ToolButtonStyles => {
  switch (thisToolType) {
    ...
    default:
      throw new Error(
        `toolButtonStyleFactory does not handle tool type ${thisToolType}`
      );
  }
};
