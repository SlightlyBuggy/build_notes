...
export const getObjectBoundaries = (inputs: GetObjectBoundariesInputs) : ObjectBoundaries => {
    switch (inputs.toolType) {
        ...
        case (DrawingTool.RadiusedCircle): 
            if(inputs.radius)
            {
                const boundaries = getRadiusedCircleBoundaries(inputs.startX, inputs.startY, inputs.radius)
                return boundaries
            }
        default:
...
const getLineBoundaries = (startX: number, startY: number, endX: number, endY: number): ObjectBoundaries => {
    ...
    const {newLeftX, newRightX, newTopY, newBottomY} = resizeBounadiresToRespectMinimums(leftX, rightX, topY, bottomY)

    if(leftX != newLeftX)
    {
        console.log(`Resized X dimensions (min, max) ${leftX}, ${rightX} -> ${newLeftX}, ${newRightX}`)
    }

    if(topY != newTopY)
        {
            console.log(`Resized Y dimensions (min, max) ${topY}, ${bottomY} -> ${newTopY}, ${newBottomY}`)
        }
...
const getRadiusedCircleBoundaries = (startX: number, startY: number, radius: number): ObjectBoundaries => {
    ...
    const {newLeftX: leftX, newRightX: rightX, newTopY: topY, newBottomY: bottomY} = resizeBounadiresToRespectMinimums(initialLeftX, initialRightX, initialTopY, initialBottomY)

    return {leftX, rightX, topY, bottomY}
}

const resizeBounadiresToRespectMinimums = (leftX: number, rightX: number, topY: number, bottomY: number) => {
    const {newSmallerDim: newLeftX, newLargerDim: newRightX } = resizeBoundaryToRespectMinimuims(leftX, rightX)
    const {newSmallerDim: newTopY, newLargerDim: newBottomY} = resizeBoundaryToRespectMinimuims(topY, bottomY)

    return {newLeftX, newRightX, newTopY, newBottomY}
}

const resizeBoundaryToRespectMinimuims = (smallerDim: number, largerDim: number) => {
